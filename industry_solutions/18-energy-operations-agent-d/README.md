...
**Industry:** Energy and Utilities · **Personas:** Plant Manager / Reliability Engineer; Compliance Manager; Sustainability Lead; Data Analyst · **Featured tools:** Dynamics 365 CcaaS, Dynamics 365 ERP, Microsoft Teams, SharePoint
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/EnergyOperationsAgentDMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/EnergyOperationsAgentDMcpAgents_1_0_0_1.zip`.
3. Open **EnergyOperationsAgentD** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Energy Operations Agent (d)` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/energy-operations-agent-d.py`](skill/energy-operations-agent-d.py) are a
framework-free skill: run `python skill/energy-operations-agent-d.py "list"`, import `query()`, or
