...
**Industry:** Cross-Industry · **Personas:** Product Manager; Engineering Lead; Director of Product · **Featured tools:** Dynamics 365 ERP, Microsoft Teams, Dynamics 365 CRM
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/ProductFeedbackInsightsAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/ProductFeedbackInsightsAgentMcpAgents_1_0_0_1.zip`.
3. Open **ProductFeedbackInsightsAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Product Feedback Insights Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/product-feedback-insights-agent.py`](skill/product-feedback-insights-agent.py) are a
framework-free skill: run `python skill/product-feedback-insights-agent.py "list"`, import `query()`, or
