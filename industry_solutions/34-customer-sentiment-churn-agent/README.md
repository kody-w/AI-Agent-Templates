...
**Industry:** Financial Services · **Personas:** Relationship Managers; Retention Specialists; Customer Success Teams · **Featured tools:** Dynamics 365 CRM, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/CustomerSentimentChurnAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/CustomerSentimentChurnAgentMcpAgents_1_0_0_1.zip`.
3. Open **CustomerSentimentChurnAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Customer Sentiment & Churn Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/customer-sentiment-churn-agent.py`](skill/customer-sentiment-churn-agent.py) are a
framework-free skill: run `python skill/customer-sentiment-churn-agent.py "list"`, import `query()`, or
