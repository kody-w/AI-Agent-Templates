...
**Industry:** Financial Services · **Personas:** Wealth Advisor; Relationship Managers; Advisory Directors · **Featured tools:** Dynamics 365 CRM, Dynamics 365 ERP, Dynamics 365 CcaaS, Microsoft Outlook
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/WealthInsightsAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/WealthInsightsAgentMcpAgents_1_0_0_1.zip`.
3. Open **WealthInsightsAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Wealth Insights Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/wealth-insights-agent.py`](skill/wealth-insights-agent.py) are a
framework-free skill: run `python skill/wealth-insights-agent.py "list"`, import `query()`, or
