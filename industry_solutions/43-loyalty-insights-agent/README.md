...
**Industry:** Cross-Industry · **Personas:** Loyalty Program Director; CRM Manager; Marketing Leader · **Featured tools:** Dynamics 365 CRM, Dynamics 365 ERP, Microsoft Outlook, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/LoyaltyInsightsAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/LoyaltyInsightsAgentMcpAgents_1_0_0_1.zip`.
3. Open **LoyaltyInsightsAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Loyalty Insights Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/loyalty-insights-agent.py`](skill/loyalty-insights-agent.py) are a
framework-free skill: run `python skill/loyalty-insights-agent.py "list"`, import `query()`, or
