...
**Industry:** Cross-Industry · **Personas:** Procurement Manager; Finance Director; Department Approver · **Featured tools:** Dynamics 365 ERP, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/PurchaseOrderManagementAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/PurchaseOrderManagementAgentMcpAgents_1_0_0_1.zip`.
3. Open **PurchaseOrderManagementAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Purchase Order Management Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/purchase-order-management-agent.py`](skill/purchase-order-management-agent.py) are a
framework-free skill: run `python skill/purchase-order-management-agent.py "list"`, import `query()`, or
