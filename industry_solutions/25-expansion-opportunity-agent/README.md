...
**Industry:** Cross-Industry · **Personas:** Sales Leader; Sales Operations Manager; Enablement Manager · **Featured tools:** Dynamics 365 CRM, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/ExpansionOpportunityAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/ExpansionOpportunityAgentMcpAgents_1_0_0_1.zip`.
3. Open **ExpansionOpportunityAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Expansion Opportunity Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/expansion-opportunity-agent.py`](skill/expansion-opportunity-agent.py) are a
framework-free skill: run `python skill/expansion-opportunity-agent.py "list"`, import `query()`, or
