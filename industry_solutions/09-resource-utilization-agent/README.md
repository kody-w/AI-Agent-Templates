...
**Industry:** Professional Services, Consulting · **Personas:** Operations Leader; Resource Manager; Finance Director · **Featured tools:** Dynamics 365 ERP, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/ResourceUtilizationAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/ResourceUtilizationAgentMcpAgents_1_0_0_1.zip`.
3. Open **ResourceUtilizationAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Resource Utilization Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/resource-utilization-agent.py`](skill/resource-utilization-agent.py) are a
framework-free skill: run `python skill/resource-utilization-agent.py "list"`, import `query()`, or
