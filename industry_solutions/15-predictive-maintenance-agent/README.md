...
**Industry:** Manufacturing · **Personas:** Maintenance Manager; Production Supervisor; Operation Leader · **Featured tools:** Dynamics 365 ERP, Dynamics 365 CcaaS, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/PredictiveMaintenanceAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/PredictiveMaintenanceAgentMcpAgents_1_0_0_1.zip`.
3. Open **PredictiveMaintenanceAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Predictive Maintenance Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/predictive-maintenance-agent.py`](skill/predictive-maintenance-agent.py) are a
framework-free skill: run `python skill/predictive-maintenance-agent.py "list"`, import `query()`, or
