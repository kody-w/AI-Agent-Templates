...
**Industry:** Retail · **Personas:** Store Associate; Sales Manager; Floor Specialist · **Featured tools:** Dynamics 365 CRM, Dynamics 365 ERP, Microsoft Teams, Microsoft Outlook
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/StoreAssociateAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/StoreAssociateAgentMcpAgents_1_0_0_1.zip`.
3. Open **StoreAssociateAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Store Associate Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/store-associate-agent.py`](skill/store-associate-agent.py) are a
framework-free skill: run `python skill/store-associate-agent.py "list"`, import `query()`, or
