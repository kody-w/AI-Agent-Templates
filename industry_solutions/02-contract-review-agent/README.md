...
**Industry:** Professional Services, Consulting · **Personas:** Legal Operations; Attorneys; Executives · **Featured tools:** SharePoint, Microsoft Word
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/ContractReviewAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/ContractReviewAgentMcpAgents_1_0_0_1.zip`.
3. Open **ContractReviewAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Contract Review Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/contract-review-agent.py`](skill/contract-review-agent.py) are a
framework-free skill: run `python skill/contract-review-agent.py "list"`, import `query()`, or
