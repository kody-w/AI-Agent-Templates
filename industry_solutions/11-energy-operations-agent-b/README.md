...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/EnergyOperationsAgentBMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/EnergyOperationsAgentBMcpAgents_1_0_0_1.zip`.
3. Open **EnergyOperationsAgentB** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Energy Operations Agent (b)` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/energy-operations-agent-b.py`](skill/energy-operations-agent-b.py) are a
framework-free skill: run `python skill/energy-operations-agent-b.py "list"`, import `query()`, or
