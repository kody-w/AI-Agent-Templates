...
**Industry:** Cross-Industry · **Personas:** Account Executive; Sales Director · **Featured tools:** Dynamics 365 CRM, SharePoint, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/SalesPipelineManagementAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/SalesPipelineManagementAgentMcpAgents_1_0_0_1.zip`.
3. Open **SalesPipelineManagementAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Sales Pipeline Management Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/sales-pipeline-management-agent.py`](skill/sales-pipeline-management-agent.py) are a
framework-free skill: run `python skill/sales-pipeline-management-agent.py "list"`, import `query()`, or
