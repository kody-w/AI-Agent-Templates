...
**Industry:** Healthcare · **Personas:** Primary care physicians; Surgeons; Anesthesia teams · **Featured tools:** Dynamics 365 ERP
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/ClinicalSummaryAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/ClinicalSummaryAgentMcpAgents_1_0_0_1.zip`.
3. Open **ClinicalSummaryAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Clinical Summary Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/clinical-summary-agent.py`](skill/clinical-summary-agent.py) are a
framework-free skill: run `python skill/clinical-summary-agent.py "list"`, import `query()`, or
