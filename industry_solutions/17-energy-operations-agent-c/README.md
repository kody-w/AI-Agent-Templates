...
**Industry:** Energy and Utilities · **Personas:** Plant Manager / Reliability Engineer; Compliance Manager; Sustainability Lead; Data Analyst · **Featured tools:** Dynamics 365 ERP, Dynamics 365 CRM, SharePoint, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/EnergyOperationsAgentCMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/EnergyOperationsAgentCMcpAgents_1_0_0_1.zip`.
3. Open **EnergyOperationsAgentC** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Energy Operations Agent (c)` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/energy-operations-agent-c.py`](skill/energy-operations-agent-c.py) are a
framework-free skill: run `python skill/energy-operations-agent-c.py "list"`, import `query()`, or
