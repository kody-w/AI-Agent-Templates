...
**Industry:** Software Tech · **Personas:** Account Executives; Customer Success Managers; Sales Leadership · **Featured tools:** Dynamics 365 CRM, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/SubscriptionRenewalAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/SubscriptionRenewalAgentMcpAgents_1_0_0_1.zip`.
3. Open **SubscriptionRenewalAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Subscription Renewal Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/subscription-renewal-agent.py`](skill/subscription-renewal-agent.py) are a
framework-free skill: run `python skill/subscription-renewal-agent.py "list"`, import `query()`, or
