...
**Industry:** Financial Services · **Personas:** Branch Bankers; Financial Advisors; Compliance Officers · **Featured tools:** Dynamics 365 CRM, Dynamics 365 ERP, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/BranchBankingAdvisoryAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/BranchBankingAdvisoryAgentMcpAgents_1_0_0_1.zip`.
3. Open **BranchBankingAdvisoryAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Branch Banking Advisory Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/branch-banking-advisory-agent.py`](skill/branch-banking-advisory-agent.py) are a
framework-free skill: run `python skill/branch-banking-advisory-agent.py "list"`, import `query()`, or
