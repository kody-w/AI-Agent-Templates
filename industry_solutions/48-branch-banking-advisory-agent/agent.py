"""Branch Banking Advisory Agent — Financial Services industry solution (AIBAST).
...
Personas: Branch Bankers; Financial Advisors; Compliance Officers.
...
class BranchBankingAdvisoryAgent(BasicAgent):
    ...
    def __init__(self):
        self.name = "BranchBankingAdvisoryAgent"
        self.metadata = {
            "name": self.name,
            "description": (
                "Automate branch banking and advisory workflows to streamline customer interactions, strengthen compliance, and improve financial guidance. Records live in Microsoft Dataverse (synthetic, no "
                ...
                "properties": {
                    "reference": {
                        ...
                        "description": ("A case reference or a subject "
                                        "name, e.g. CS-10960 or 'Contoso'. Pass 'list' to "
                                        "see every case.")},
                },
                "required": [],
            },
    ...
    def perform(self, **kwargs):
        ...
        if not ref or ref.lower() == "list":
            lines = ["## Branch Banking Advisory Agent — case records (Dataverse)"]
