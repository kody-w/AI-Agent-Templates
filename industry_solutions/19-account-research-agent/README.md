...
**Industry:** Cross-Industry · **Personas:** Account Executive; Sales Director; Customer Success Manager · **Featured tools:** Dynamics 365 CRM, SharePoint, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/AccountResearchAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/AccountResearchAgentMcpAgents_1_0_0_1.zip`.
3. Open **AccountResearchAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Account Research Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/account-research-agent.py`](skill/account-research-agent.py) are a
framework-free skill: run `python skill/account-research-agent.py "list"`, import `query()`, or
