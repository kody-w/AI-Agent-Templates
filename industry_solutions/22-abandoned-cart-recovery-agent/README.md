...
**Industry:** Cross-Industry, Software Tech · **Personas:** Marketing Manager; Digital Marketing Lead; Growth Manager · **Featured tools:** Dynamics 365 ERP, Microsoft Outlook, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/AbandonedCartRecoveryAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/AbandonedCartRecoveryAgentMcpAgents_1_0_0_1.zip`.
3. Open **AbandonedCartRecoveryAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Abandoned Cart Recovery Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/abandoned-cart-recovery-agent.py`](skill/abandoned-cart-recovery-agent.py) are a
framework-free skill: run `python skill/abandoned-cart-recovery-agent.py "list"`, import `query()`, or
