...
**Industry:** Retail, Manufacturing · **Personas:** Supply Chain Planner; Operations Leader; Procurement Manager · **Featured tools:** Dynamics 365 ERP, Microsoft Teams
...
1. **Copilot Studio → Solutions → Import solution** → `copilot_studio_solution/SupplyChainRiskAgentMcpConnectors_1_0_0_1.zip` → Import.
2. Repeat for `copilot_studio_solution/SupplyChainRiskAgentMcpAgents_1_0_0_1.zip`.
3. Open **SupplyChainRiskAgent** and **Publish**.
4. **Tools → Add a tool → Model Context Protocol** → the `Supply Chain Risk Agent` connector → no-auth connection → Add → Publish.
...
[`skill/SKILL.md`](skill/SKILL.md) + [`skill/supply-chain-risk-agent.py`](skill/supply-chain-risk-agent.py) are a
framework-free skill: run `python skill/supply-chain-risk-agent.py "list"`, import `query()`, or
